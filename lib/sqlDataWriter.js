...
class SqlDataWriter extends Writable {
   ...
   */
  constructor (connectionString, collectionName, containerMode, updateExisting = undefined) {
    super({
      objectMode: true
    })
  ...
  _write (chunk, encoding, callback) {
    ...
    .transaction((transaction) => {
      if (this._containerMode) {
        serializeContainerToSql(transaction, this._collectionName, chunk, this._updateExisting)
      } else {
        serializeObjectToSql(transaction, this._collectionName, chunk, this._updateExisting)
      }
    })
    .then(() => {
      callback()
