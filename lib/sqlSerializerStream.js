...
class SqlSerializerStream extends Transform {
   ...
   * @param {Object} [metadata=] Collection metadata to use
   * @param {SqlSerializerStream~Options} [options=] Options
   */
        ...
        _.forOwn(this._metadata || chunk, (entity, entityName) => {
          ...
          if (this._metadata) {
            ...
            if (_.isFunction(this._metadata)) {
              entityMetadata = this._metadata(entityName)
            } else if (_.isPlainObject(this._metadata[entityName])) {
              entityMetadata = _.assign(entityMetadata || {}, this._metadata[entityName])
            }
          }
          if (!entityMetadata) {
            throw new Error(`No metadata for '${entityName}'`)
          }
          this._prologueMetadata[entityName] = entityMetadata

          const tableName = this._collectionPrefix
            ? `${this._collectionPrefix}${entityName}`
            : entityName

          const result = this._prologue(this._serializer, pgEscape.ident(tableName), entityMetadata, queries)
