...
class SqlSerializer {
   ...
   * @param {SchemaFlatter~Metadata|function} [metadata=] Collection metadata to use
  ...
  serializeData (data, collectionPrefix = undefined, metadata = undefined, queries = undefined) {
    ...
    _
      ...
      .sort((l, r) => {
        let lMetadata
        if (data[l]) {
          lMetadata = data[l].metadata
        }
        if (metadata) {
          lMetadata = lMetadata ? _.cloneDeep(lMetadata) : lMetadata
          if (_.isFunction(metadata)) {
            lMetadata = metadata(l)
          } else if (_.isPlainObject(metadata[l])) {
            lMetadata = _.assign(lMetadata || {}, metadata[l])
          }
        }
        if (!lMetadata) {
          throw new Error(`No metadata for '${l}'`)
        }

        let rMetadata
        if (data[r]) {
          rMetadata = data[r].metadata
        }
        if (metadata) {
          rMetadata = rMetadata ? _.cloneDeep(rMetadata) : rMetadata
          if (_.isFunction(metadata)) {
            rMetadata = metadata(r)
          } else if (_.isPlainObject(metadata[r])) {
            rMetadata = _.assign(rMetadata || {}, metadata[r])
          }
        }
        if (!rMetadata) {
          throw new Error(`No metadata for '${r}'`)
        }
      ...
      .forEach((entityName) => {
        ...
        if (metadata) {
          entityMetadata = entityMetadata ? _.cloneDeep(entityMetadata) : entityMetadata
          if (_.isFunction(metadata)) {
            entityMetadata = metadata(entityName)
          } else if (_.isPlainObject(metadata[entityName])) {
            entityMetadata = _.assign(entityMetadata || {}, metadata[entityName])
          }
        }
        if (!entityMetadata) {
